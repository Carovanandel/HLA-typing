...
formatted_path = '/exports/me-lcco-aml-hpc/cavanandel/HLA-typing/output-analysis/formatted-hla-type.csv'
...
output_path = '/exports/me-lcco-aml-hpc/cavanandel/HLA-typing/output-analysis/correct-nomen-hla-type.csv'
output_open = open(output_path, 'w', newline='')
output_writer = csv.DictWriter(output_open, delimiter=',', fieldnames=header_full)
output_writer.writeheader()

incorrect_path = '/exports/me-lcco-aml-hpc/cavanandel/HLA-typing/output-analysis/incorrect-nomen-hla-type.txt'
incorrect_file = open(incorrect_path, 'w')
incorrect_file.write('Alleles with incorrect nomenclature:\n')


#create empty output row, 'X' will be left for alleles for which an hla class cannot be created
output_row = {column: 'X' for column in header_full}
...
for row in formatted_reader:
    ...
    for allele in header_full[1:]:
        ...
        for option in options:
            try: 
                if option == '0':
                    hla = str(HLA(None))
                else: hla = str(HLA.from_str(option)) #test if hla class can be created, then return string again
            except ValueError:
    ...
    output_row = {column: 'X' for column in header_full} #clear output row
